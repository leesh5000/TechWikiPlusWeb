...
function VerifyEmailContent() {
  ...
  const handleSubmit = async (verificationCode: string) => {
    ...
    try {
      ...
      // 성공 시에만 로그인 페이지로 이동 (auth-context에서 처리됨)
    } catch (err: any) {
      // 에러 시 토스트 팝업 표시하고 페이지에 머무름 - 서버 에러 메시지 그대로 표시
      const errorMessage = err.message || '인증 코드 확인에 실패했습니다.'
      showError('인증 실패', errorMessage)
      setError(errorMessage)
      setIsLoading(false)
      
      // 입력 필드 초기화하고 첫 번째 필드에 포커스
      setCode(['', '', '', '', '', ''])
      setTimeout(() => {
  ...
  const handleResend = async () => {
    ...
    try {
      ...
      setCode(['', '', '', '', '', ''])
      inputRefs.current[0]?.focus()
      showSuccess('인증 코드가 재전송되었습니다.', '이메일을 확인해주세요.')
    } catch (err: any) {
      showError('인증 코드 재전송에 실패했습니다.', err.message)
